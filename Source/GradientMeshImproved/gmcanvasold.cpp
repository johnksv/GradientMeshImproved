#include "gmcanvasold.h"
#include <QMouseEvent>
#include <QDebug>

GMCanvasOLD::GMCanvasOLD(QWidget *parent) :
    QOpenGLWidget{parent}
{
    setMouseTracking(true);
}

void GMCanvasOLD::initializeGL()
{
...
void GMCanvasOLD::resizeGL(int, int)
{
    glOrtho(0, glWidth, 0, glHeight, -1.0, 1.0);
}

void GMCanvasOLD::setRenderingMode(int mode){
    renderingMode = mode;
}
void GMCanvasOLD::setDrawingMode(drawModeCanvas drawMode){
    this->drawMode = drawMode;
}

void GMCanvasOLD::paintGL()
{    
    QPainter qPainter;
...
void GMCanvasOLD::handleFileDialog(QString location, bool import){
    if(import){
...
void GMCanvasOLD::mousePressEvent(QMouseEvent* event){
    int x = event->x();
    int y = event->y();
...
void GMCanvasOLD::mouseMoveEvent(QMouseEvent *event){
    oldMousePos = event->pos();
    update();
}

void GMCanvasOLD::wheelEvent(QWheelEvent *event){
